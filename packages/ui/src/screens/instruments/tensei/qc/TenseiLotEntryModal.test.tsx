...
import { TenseiLotEntryModal, TestId } from "./TenseiLotEntryModal";
import { TestId as ContentTestId } from "./TenseiQcLotEntryContent";
import {
  randomInstrumentDto,
...
describe("Tensei Lot entry modal", () => {
  describe("next button", () => {
    ...
    it("disables the next button if 1 level is complete but another is partially entered", async () => {
      ...
      const nextResponse: TenseiQcBarcodeValidateResponseDto = {
        ...firstResponse,
  ...
  describe("barcode validation", () => {
    ...
    it("displays error message when there's a 200 response with no currentBarcodeStatus field", async () => {
      ...
      server.use(
        rest.put("**/tensei/qc/barcodes/validate", (req, res, context) =>
...
function mockBarcodeResponse(
  response: Partial<TenseiQcBarcodeValidateResponseDto>
) {
  const fullResponse: TenseiQcBarcodeValidateResponseDto = {
    barcodes: [],
    currentBarcodeStatus: CurrentBarcodeStatusEnum.BARCODEERROR,
...
const randomBarcode = (
  barcode: Partial<TenseiQcBarcodeResponseDto>
): TenseiQcBarcodeResponseDto => ({
  barcode: "",
  sequenceNumber: 1,
