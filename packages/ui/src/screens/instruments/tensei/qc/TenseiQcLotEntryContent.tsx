...
import type { AcceptedBarcodes } from "./TenseiLotEntryModal";
...
import { QC_BARCODE_LENGTH } from "../../../../api/TenseiApi";

export const TestId = {
  BarcodeInput: "tensei-qc-lot-barcode-input",
  ErrorText: "tensei-qc-lot-error-text",
  BarcodeStatus: (sequence: number) =>
    `tensei-qc-lot-barcode-status-${sequence}`,
};
...
export function TenseiQcLotEntryContent(props: LotEntryContentProps) {
  ...
  return (
    <LotEntryRoot>
      <SpotText level="paragraph">
        <Trans i18nKey="instrumentScreens.tensei.qualityControl.addQcLot.instructions" />
      </SpotText>
      ...
      <EntrySection>
        <SummaryBox>
          <SpotText level="paragraph">
            {t(
              "instrumentScreens.tensei.qualityControl.addQcLot.barcodesHeader"
            )}
          </SpotText>

          <SpotText level="paragraph" bold>
            {t("instrumentScreens.tensei.qualityControl.addQcLot.levelTwo")}
          </SpotText>
          ...
          <SpotText level="paragraph" bold>
            {t("instrumentScreens.tensei.qualityControl.addQcLot.levelOne")}
          </SpotText>
        ...
        <Section>
          <div>
            ...
            >
              <ValidationError>
                {t("instrumentScreens.tensei.qualityControl.addQcLot.error")}
              </ValidationError>
            </ValidationErrorContainer>
          </div>
  ...
  return (
    <BarcodeLabelContainer validated={props.validationStatus != null}>
      ...
      <SpotText level="paragraph" bold={props.isNext}>
        {t(
          "instrumentScreens.tensei.qualityControl.addQcLot.barCodePlaceholder",
    ...
    <BarcodeStatusContainer data-testid={TestId.BarcodeStatus(props.sequence)}>
      {props.validationStatus === BarcodeStatusEnum.ACCEPTED && (
        <SpotText level="secondary">
          {t(
            "instrumentScreens.tensei.qualityControl.addQcLot.barcodeAccepted"
          )}
        </SpotText>
      )}
      ...
      {props.isNext && (
        <SpotText level="paragraph" bold>
          {t("instrumentScreens.tensei.qualityControl.addQcLot.enterBarcode")}
        </SpotText>
      )}
