...
const InfoGrid = (props: InfoGridProps) => {
  ...
  return (
    <InfoGridRoot className={props.className}>
      ...
      <SpotText level="paragraph" bold>
        {t("instrumentScreens.tensei.qc.level")}:
      </SpotText>
      <SpotText level="paragraph" data-testid={TestIds.level}>
        {qcDto.level}
      </SpotText>
      <SpotText level="paragraph" bold>
        {t("instrumentScreens.tensei.qc.unopenedExpiration")}:
      </SpotText>
...
const RefRangeTable = (props: RefRangeTableProps) => {
  ...
  const columns = useMemo(
    () => [
      {
        Header: t("instrumentScreens.tensei.qc.parameter"),
        accessor: "assayName",
      },
      {
        Header: t("instrumentScreens.tensei.qc.target"),
        accessor: "target",
      },
      {
...
interface TenseiQcLotInfoModalProps
  extends Omit<
    BasicModalProps,
...
const TenseiQcLotInfoModal = (props: TenseiQcLotInfoModalProps) => {
  const { t } = useTranslation();

  return (
    <StyledModal
      data-testid="tensei-lot-view-modal"
      open={props.open}
...
export type { TenseiQcLotInfoModalProps };
export { TenseiQcLotInfoModal };
