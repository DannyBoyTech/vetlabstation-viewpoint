...
import { TenseiLotEntryModal } from "./TenseiLotEntryModal";
import { TenseiQcLotInfoModal } from "./TenseiQcLotInfoModal";
import { useGetQcLotsQuery } from "../../../../api/QualityControlApi";
import { skipToken } from "@reduxjs/toolkit/query";
import { TenseiRunQcModal } from "./TenseiRunQcModal";
import { TenseiExpiredQcWarningModal } from "./TenseiExpiredQcWarningModal";
  ...
  return (
    <InstrumentPageRightPanelButtonContainer>
      ...
      <Button onClick={props.onAddQCLot} data-testid={TestId.AddQCLotButton}>
        {t("instrumentScreens.general.buttons.addQCLot")}
      </Button>
      <StyledButton
...
const TenseiQcScreen = ({ instrument }: TenseiQcScreenProps) => {
  ...
  const columns = useMemo(
    () => [
      {
        Header: t("instrumentScreens.tensei.labels.lotNumber"),
        accessor: "lot",
      },
      {
        Header: t("instrumentScreens.tensei.labels.level"),
        accessor: "level",
      },
      {
  ...
  const handleCancelRunExpiredQc = () => {
    setExpiredQcWarningOpen(false);
  };

  const handleAddQCLot = () => setLotEntryOpen(true);

  const handleViewQCResults = () => {
    // Mock implementation
  };
  ...
  return (
    <Root>
      ...
      {expiredQcWarningOpen && selectedQc != null && (
        <TenseiExpiredQcWarningModal
          open={expiredQcWarningOpen}
          onClose={handleCancelRunExpiredQc}
...
export type { TenseiQcScreenProps };
export { TenseiQcScreen };
